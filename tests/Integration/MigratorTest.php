...
final class MigratorTest extends DatabaseTestCase
{
    public function testMigrationCreatesExpectedTablesAndIsIdempotent(): void
    {
        ...
        self::assertSame([
            'account_niches', 'account_subniches', 'affiliate_media_declarations', 'discovery_run_entries', 'discovery_runs', 'installations',
            'login_attempts', 'ml_categories',
            'ml_credentials', 'ml_oauth_states', 'niches', 'schema_migrations', 'subniche_categories', 'subniches', 'user_sessions', 'users',
        ], $tables);
        ...
        self::assertSame(['0001', '0002', '0003', '0004', '0005', '0006'], $pdo->query('SELECT version FROM schema_migrations ORDER BY version')->fetchAll(\PDO::FETCH_COLUMN));
    }
    ...
    public function testDatabaseWithOnly0001IsUpgradedTo0002(): void
    {
        ...
        try {
            ...
            self::assertSame(['0002', '0003', '0004', '0005', '0006'], (new Migrator($pdo, $migrations))->migrate());
